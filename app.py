...
st.set_page_config(layout="wide", page_title='Amino Acid Property Calculator', page_icon=':page_with_curl:')

st.title('Amino Acid Properties :page_with_curl:')
...
with st.sidebar:
    ...
    st.subheader('Terminus')
    c1, c2 = st.columns(2)
    nterm = c1.checkbox('N-term', value=False, help='Include N-terminus (+H) in calculations.')
    cterm = c2.checkbox('C-term', value=False, help='Include C-terminus (+OH) in calculations.')
    ...
    precision = st.number_input('Decimal places', min_value=0, max_value=10, value=5, step=1,
                                help='Number of decimal places to round the masses to.')
...
st.dataframe(
    ...
    height=950,
    column_order=("Name", "3 letter code", "1 letter code", "Monoisotopic Mass", "Average Mass", "Composition", "gif")
)
